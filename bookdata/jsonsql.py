...
for i in range(len(data['data'])):
    ...
    keyword = (data['data'][i]['summary'][:250] + '...') if len(data['data'][i]['summary']) > 250 else data['data'][i][
        'summary']
    try:
    ...
    try:
        ...
        isbn13, isbn10, title, author, publisher, year, num_copies, price, book_format, keyword, book_subject))
        db.commit()
    except:
        db.rollback()
